...
from util import read_corpus, data_iter, batch_slice, map_ent_label
from vocab import Vocab, VocabEntry
from process_samples import generate_hamming_distance_payoff_distribution
import math

from rougescore import *
from nli_models.baseline_snli_one import *
...
def init_config():
    ...
    parser.add_argument('--switch', default=10, type=int, help='multi-task switch')
    ...
    parser.add_argument('--train_ent_x', type=str, help='path to the training entailment x file')
    parser.add_argument('--train_ent_y', type=str, help='path to the training entailment y file')
    parser.add_argument('--train_ent_label', type=str, help='path to the training entailment label file')
    ...
    parser.add_argument('--valid_niter', default=2000, type=int, help='every n iterations to perform validation')
    parser.add_argument('--valid_metric', default='rouge2_f', choices=['rouge2_f', 'bleu', 'ppl', 'word_acc', 'sent_acc'], help='metric used for validation')
    ...
    parser.add_argument('--drop_patience', default=12, type=int, help='training lr drop patience')
    ...
    parser.add_argument('--reward_type', default='entailment', type=str, choices=['rouge2_f', 'bleu', 'entailment'])
...
def sentence_entailment(src, targ, word_dic, input_encoder, inter_atten):
    ...
    src = ['<s>'] + src
    targ = ['<s>'] + targ
    src = [word_dic[w] for w in src if w in word_dic]
    targ = [word_dic[w] for w in targ if w in word_dic]
    source = Variable((torch.from_numpy(np.array(src, dtype=int)) - 1).cuda())
    target = Variable((torch.from_numpy(np.array(targ, dtype=int)) - 1).cuda())
    test_src_linear, test_tgt_linear=input_encoder(source, target)
    log_prob=inter_atten(test_src_linear, test_tgt_linear)
    scores = F.softmax(log_prob).data.cpu().numpy()[0]
...
class NMT(nn.Module):
    def __init__(self, args, vocab):
        ...
        self.decoder_hidden_init = nn.Linear(args.hidden_size * 2, args.hidden_size)

        self.classifier = nn.Sequential(
            nn.Dropout(p=args.dropout),
            nn.Linear(args.hidden_size * 8, 512),
            nn.Tanh(),
            nn.Dropout(p=args.dropout),
            nn.Linear(512, 512),
            nn.Tanh(),
    ...
    def forward(self, task, src_sents, src_sents_len, tgt_words, tgt_sents_len):
        if task == "summarization":
            src_encodings, sv, init_ctx_vec = self.encode(src_sents, src_sents_len)
            scores = self.decode(src_encodings, init_ctx_vec, tgt_words)
        elif task == "entailment":
            src_encodings, src_sentence_vectors, init_ctx_vec_src = self.encode(src_sents, src_sents_len)
            tgt_sentence_vectors = self.encode_sort(tgt_words, tgt_sents_len)
            features = torch.cat((src_sentence_vectors, tgt_sentence_vectors, torch.abs(src_sentence_vectors - tgt_sentence_vectors), \
                                  src_sentence_vectors * tgt_sentence_vectors), 1)
            scores = self.classifier(features)

        return scores

    def encode_sort(self, src_sents, src_sents_len):
        """
        :param src_sents: (src_sent_len, batch_size), sorted by the length of the source
        :param src_sents_len: (src_sent_len)
        """
        src_sents_len_sort, idx_sort = np.sort(src_sents_len)[::-1], np.argsort(src_sents_len)[::-1]
        idx_sort = list(idx_sort)
        src_sents_sort = src_sents.index_select(1, Variable(torch.cuda.LongTensor(idx_sort)))
        #src_sents_sort = torch.index_select(src_sents, 1, Variable(torch.cuda.LongTensor(idx_sort)))

        # (src_sent_len, batch_size, embed_size)
        src_word_embed = self.src_embed(src_sents_sort)
        packed_src_embed = pack_padded_sequence(src_word_embed, src_sents_len_sort)

        # output: (src_sent_len, batch_size, hidden_size)
        output, (last_state, last_cell) = self.encoder_lstm(packed_src_embed)

        sentence_vector = torch.cat([last_state[0], last_state[1]], 1)
        idx_unsort = np.argsort(idx_sort)
        sentence_vector = sentence_vector.index_select(0, Variable(torch.cuda.LongTensor(idx_unsort)))
    ...
    def encode(self, src_sents, src_sents_len):
        ...
        #dec_init_state = F.tanh(self.decoder_init(torch.cat([last_state[0], last_state[1]], 1)))
        dec_init_cell = F.tanh(self.decoder_cell_init(torch.cat([last_cell[0], last_cell[1]], 1)))
        dec_init_state = F.tanh(self.decoder_hidden_init(torch.cat([last_state[0], last_state[1]], 1)))

        sentence_vector = torch.cat([last_state[0], last_state[1]], 1)

        return output, sentence_vector, (dec_init_state, dec_init_cell)
    ...
    def translate(self, src_sents, beam_size=None, to_word=True):
        ...
        src_encoding, sv, dec_init_vec = self.encode(src_sents_var, [len(src_sents[0])])
        src_encoding_att_linear = tensor_transform(self.att_src_linear, src_encoding)
    ...
    def sample(self, src_sents, sample_size=None, to_word=False):
        ...
        src_sents_var = to_input_variable(src_sents, self.vocab.src, cuda=args.cuda, is_test=True)
        src_encoding, sv, (dec_init_state, dec_init_cell) = self.encode(src_sents_var, [len(s) for s in src_sents])
...
def evaluate_loss(model, data, crit):
    ...
    for src_sents, tgt_sents in data_iter(data, batch_size=args.batch_size, shuffle=False):
        ...
        scores = model('summarization', src_sents_var, src_sents_len, tgt_sents_var[:-1], src_sents_len)
...
def init_training(args):
    ...
    loss_ent = nn.CrossEntropyLoss(weight=weight, size_average=False)

    if args.cuda:
        model = model.cuda()
        nll_loss = nll_loss.cuda()
        cross_entropy_loss = cross_entropy_loss.cuda()
        loss_ent = loss_ent.cuda()

    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)

    return vocab, model, optimizer, nll_loss, cross_entropy_loss, loss_ent

def cont_training_halve_raml(args, lr_now):
    load_model_dir = args.save_to + '.bin'
    print('load model from [%s]' % load_model_dir, file=sys.stderr)
    params = torch.load(load_model_dir, map_location=lambda storage, loc: storage)
    vocab = params['vocab']
...
def cont_training_halve_ent(args, lr_now):
    load_model_dir = args.save_to + '.bin'
    print('load model from [%s]' % load_model_dir, file=sys.stderr)
    params = torch.load(load_model_dir, map_location=lambda storage, loc: storage)
    vocab = params['vocab']
...
def train(args):
    ...
    train_data_ent_x = read_corpus(args.train_ent_x, source='src')
    train_data_ent_y = read_corpus(args.train_ent_y, source='src')
    train_data_ent_label = read_corpus(args.train_ent_label, source='src')
    train_data_ent_label = map_ent_label(train_data_ent_label)

    dev_data_src = read_corpus(args.dev_src, source='src')
    dev_data_tgt = read_corpus(args.dev_tgt, source='tgt')

    train_data = zip(train_data_src, train_data_tgt)
    train_data_ent = zip(train_data_ent_x, train_data_ent_y, train_data_ent_label)

    dev_data = zip(dev_data_src, dev_data_tgt)

    if not args.load_model:
        vocab, model , optimizer, nll_loss, cross_entropy_loss, loss_ent = init_training(args)
    else:
        vocab, model, optimizer, nll_loss, cross_entropy_loss, loss_ent = cont_training(args)

    train_iter = patience = drop_patience = cum_loss = report_loss = cum_tgt_words = report_tgt_words = 0
    cum_examples = cum_batches = report_examples = valid_num = best_model_iter = 0
    train_multi_task_iter = 0
    train_ent_iter = 0
    hist_valid_scores = []
    train_time = begin_time = time.time()
    print('begin Maximum Likelihood training')

    sum_data_generator = data_iter(train_data, batch_size=args.batch_size)
    ent_data_generator = data_iter(train_data_ent, batch_size=args.batch_size)

    while True:
        train_multi_task_iter += 1
        for train_iter in range((train_multi_task_iter - 1)  * 100, train_multi_task_iter * 100):
            try:
                src_sents, tgt_sents = next(sum_data_generator)
            except:
                sum_data_generator = data_iter(train_data, batch_size=args.batch_size)
                src_sents, tgt_sents = next(sum_data_generator)
            ...
            if train_iter > 0 and train_iter % args.log_every == 0:
            ...
            # perform validation
            if train_iter > 0 and train_iter % args.valid_niter == 0:
                ...
                if args.valid_metric in ['rouge2_f', 'bleu', 'word_acc', 'sent_acc']:
                    ...
                    elif args.valid_metric == 'rouge2_f':
                        valid_metric = get_rouge2f([tgt for src, tgt in dev_data], dev_hyps)
                    else:
                ...
                if is_better:
                    ...
                    drop_patience += 1
                    print('hit drop patience %d' % drop_patience, file=sys.stderr)
                    if drop_patience == args.drop_patience and args.lr_decay:
                        drop_patience = 0
                        lr = optimizer.param_groups[0]['lr'] * args.lr_decay
                        vocab, model, optimizer, nll_loss, cross_entropy_loss, loss_ent = cont_training_halve_ent(args, lr)
                        print('decay learning rate to %.12f' % lr, file=sys.stderr)
                    ...
                    if patience == args.patience:
                        ...
                        exit(0)
        for train_ent_iter in range((train_multi_task_iter - 1)  * args.switch, train_multi_task_iter * args.switch):
            try:
                ent_xs, ent_ys, ent_ls = next(ent_data_generator)
            except:
                ent_data_generator = data_iter(train_data_ent, batch_size=args.batch_size)
                ent_xs, ent_ys, ent_ls = next(ent_data_generator)

            ent_xs_var = to_input_variable(ent_xs, vocab.src, cuda=args.cuda)
            ent_ys_var = to_input_variable(ent_ys, vocab.src, cuda=args.cuda)
            ent_ls_var = Variable(torch.LongTensor(ent_ls), volatile=False, requires_grad=False)
            ent_ls_var = torch.squeeze(ent_ls_var)
            if args.cuda:
                ent_ls_var = ent_ls_var.cuda()

            batch_size = len(ent_xs)
            ent_xs_len = [len(s) for s in ent_xs]
            ent_ys_len = [len(s) for s in ent_ys]

            optimizer.zero_grad()

            # (tgt_sent_len, batch_size, tgt_vocab_size) -> (batch_size)
            scores = model('entailment', ent_xs_var, ent_xs_len, ent_ys_var, ent_ys_len)
            loss = loss_ent(scores, ent_ls_var) / batch_size

            loss.backward()
            # clip gradient
            grad_norm = torch.nn.utils.clip_grad_norm(model.parameters(), args.clip_grad)
            optimizer.step()
        if train_ent_iter > 0 and ((train_ent_iter + 1 )* 100 / args.switch) % args.log_every == 0:
            print('iter %d entailment classifier training complete .' % (train_ent_iter + 1))
...
def train_raml(args):
    ...
    vocab, model, optimizer, nll_loss, cross_entropy_loss = cont_training_raml(args)
    ...
    if args.reward_type == 'entailment':
        word_dic = {}
        for line in open("/path/to/nli/word/dict").readlines():
            v, k = line.strip().split()
            word_dic[v] = int(k)
        input_encoder = encoder(137665, 300, 300, 0.01).cuda()
        inter_atten = atten(300, 3, 0.01).cuda()
        input_encoder.load_state_dict(torch.load('/path/to/input-encoder.pt', map_location=lambda storage, loc: storage))
        inter_atten.load_state_dict(torch.load('/path/to/inter-atten.pt', map_location=lambda storage, loc: storage))
        input_encoder.eval()
        inter_atten.eval()
                ...
                for src_sent, tgt_sent in zip(src_sents, tgt_sents):
                    ...
                    for i, e in enumerate(e_samples):
                        ...
                        if args.raml_sample_mode == 'hamming_distance_impt_sample':
                            if e > 0:
                                # remove <s> and </s>
                                if args.reward_type == 'bleu':
                                    reward_score = sentence_bleu([tgt_ref_tokens], new_tgt_sent[1:-1], smoothing_function=sm_func)
                                elif args.reward_type == 'rouge2_f':
                                    #print(tgt_ref_tokens)
                                    #print(new_tgt_sent[1:-1])
                                    #print(aa)
                                    reward_score = rouge_2(tgt_ref_tokens, new_tgt_sent[1:-1])
                                elif args.reward_type == 'entailment':
                                    reward_score_tgt = sentence_entailment(src_sent[1:-1], tgt_sent[1:-1],
                                                                       word_dic, input_encoder, inter_atten)
                                    reward_score_sample = sentence_entailment(src_sent[1:-1], new_tgt_sent[1:-1],
                                                                       word_dic, input_encoder, inter_atten)
                                    reward_score = min(reward_score_tgt, reward_score_sample)
                                bleu_scores.append(reward_score)
                            else:
            ...
            if train_iter % args.valid_niter == 0:
                ...
                if is_better:
                    ...
                    if drop_patience == args.drop_patience and args.lr_decay:
                        drop_patience = 0
                        lr = optimizer.param_groups[0]['lr'] * args.lr_decay
                        vocab, model, optimizer, nll_loss, cross_entropy_loss = cont_training_halve_raml(args, lr)
                        print('decay learning rate to %.12f' % lr, file=sys.stderr)
...
def get_rouge2f(references, hypotheses):
    ...
    rouge2f_score = rouge_2_corpus([ref[1:-1] for ref in references],
                             [hyp[1:-1] for hyp in hypotheses])
...
def sample(args):
    ...
    for src_sents, tgt_sents in data_iter(train_data, batch_size=args.batch_size):
        ...
        if train_iter > 0 and train_iter % check_every == 0:
